...
| P2-X | Independent AI Collab repository extraction | `in_progress_validation`：private `AtomGradient/ai-collab`已建立，产品code anchor为`f1ea8cf8baa5fb53d7afe6d02c26c3cb1378554b`、EdgeStudio code anchor为`e07a71859ea21119fa0e7e9bf70a31b70f851da2`、consumer为`a619b355a1ae03ae1c48921c89ebf989f7e8eadc`；通用core/CLI/App/HostAgent/driver/PingAgent/tooling/contract/tests/docs已迁入，consumer/gate anchors已切换。当前产品242 passed；上一向量root Harness1399、consumer242 passed、Swift14 executed（1 expected skip）/0 failures、PingAgent5+7 passed。真实四TUI回归先后暴露vendor tool login shell不继承动态PATH/client env，以及Host签发的module root多取一层；产品现以owner-private generation-scoped exact入口和clean-env exact-interpreter import共同守卫，失败witness不计acceptance | 重新审核最终fixed vector；重建并验证final signed App/embedded Host；Codex与Claude分别执行真实多Scenario/多Agent delivery/Close/Resume/no-cross/destroy完整回归。公开前另做license/header/secret/doc/notarization裁决；旧Scenario仅兼容读取，不迁移durable state | product package/App/PingAgent/docs；EdgeStudio manifest/adapter/build wrappers/gates；edge-studio-dev compatibility；all acceptance surfaces | superseding fixed-SHA cross-repo review；两套独立全量测试与真实Scenario evidence；通过后才进入employee acceptance |
