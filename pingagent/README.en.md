...
- **Harness participant mode**: A Harness-launched participant keeps using `ai-ping <to>` and `--reply-to <delivery-id>`, but the intent returns to Host through a Host-issued scoped context. Host authenticates the sender with the local IPC peer PID and the exact owned descendant process chain. Once Host durably persists the route/envelope, the command returns a compact `accepted` result; Host independently supervises dispatch, delivery/consumption ACKs, retry, and restart recovery. The complete message is written to `.ai-mailbox/inbox/<role>/<delivery-id>.md` in the receiver's Scenario working directory. If the project does not already ignore that directory, the adapter appends `.ai-mailbox/` to the Scenario clone's `.gitignore`; registration never modifies the user's original checkout. Exact-session injection carries only a short `[ai-collab 收信] ... Read <path>` pointer, so long bodies never pass through the TUI input path. This Host-managed mailbox is not used for role discovery or watching. Normal ACKs stay in machine state/UI/audit instead of being reinjected as “peer received it” Agent messages, triggering model inference, or creating ACK loops. This mode rejects `--from`, never falls back to a legacy watcher mailbox, and leaves explicit waiting/querying to the Host delivery read model instead of legacy `--wait`.
